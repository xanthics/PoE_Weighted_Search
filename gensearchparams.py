...
def gensearchparams(dps, selections, base):
	...
	modstr = {
		...
		"#% increased Attack Speed if you've dealt a Critical Strike Recently": [dps['attackspeed'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['attackspeed'][1]],
		...
		"#% increased Cast Speed if you've dealt a Critical Strike Recently": [dps['castspeed'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['castspeed'][1]],
		...
		"#% increased Cast Speed if you've Killed Recently": [dps['castspeed'][0] if {'conditionKilledRecently'}.issubset(selections) else 0, dps['castspeed'][1]],
		...
		# Damage - Conditional
		"Vaal Skills deal #% increased Damage": [dps['pgeneric'][0] if {'Vaal'}.issubset(selections) else 0, dps['pgeneric'][1]],
		...
		"#% increased Projectile Attack Damage during any Flask Effect": [dps['pgeneric'][0] if {'Projectile', "Attack", 'conditionUsingFlask'}.issubset(selections) else 0, dps['pgeneric'][1]],
		...
		"#% increased Elemental Damage if you've dealt a Critical Strike Recently": [dps['pelemental'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['pelemental'][1]],
		"#% increased Damage if you've Killed Recently": [dps['pgeneric'][0] if {'conditionKilledRecently'}.issubset(selections) else 0, dps['pgeneric'][1]],
		"#% increased Damage with Hits against Chilled Enemies": [dps['pgeneric'][0] if {'conditionEnemyChilled'}.issubset(selections) and {'Attack', 'Spell'}.intersection(selections) else 0, dps['pgeneric'][1]],
		# Double Damage
		...
		"#% increased Critical Strike Chance against Poisoned Enemies": [dps['critchance'][0] if {'conditionEnemyPoisoned'}.issubset(selections) else 0, dps['critchance'][1]],
		"#% increased Critical Strike Chance against Shocked Enemies": [dps['critchance'][0] if {'conditionEnemyShocked'}.issubset(selections) else 0, dps['critchance'][1]],
		...
		"#% increased Critical Strike Chance if you have Killed Recently": [dps['critchance'][0] if {'conditionKilledRecently'}.issubset(selections) else 0, dps['critchance'][1]],
		...
		"#% to Critical Strike Multiplier if you've Killed Recently": [dps['critmulti'][0] if {'conditionKilledRecently'}.issubset(selections) else 0, dps['critmulti'][1]],
		...
		"Adds # to # Chaos Damage to Spells while Dual Wielding": [dps['flatchaos'][0] if {'Spell', 'DualWielding'}.issubset(selections) else 0, dps['flatchaos'][1]],
		"Adds # to # Chaos Damage to Spells while holding a Shield": [dps['flatchaos'][0] if {'Spell', 'Shield'}.issubset(selections) else 0, dps['flatchaos'][1]],
		"Adds # to # Chaos Damage to Spells while wielding a Two Handed Weapon": [dps['flatchaos'][0] if {'Spell', 'TwoHandedWeapon'}.issubset(selections) else 0, dps['flatchaos'][1]],
		"Adds # to # Chaos Damage": [dps['flatchaos'][0], dps['flatchaos'][1]],
		"Adds # to # Chaos Damage if you've dealt a Critical Strike Recently": [dps['flatchaos'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['flatchaos'][1]],
		# Flat Damage - Cold
		"Adds # to # Cold Damage to Attacks": [dps['flatcold'][0] if {'Attack'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Axe Attacks": [dps['flatcold'][0] if {'Attack', 'Axe'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Bow Attacks": [dps['flatcold'][0] if {'Attack', 'Bow'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Claw Attacks": [dps['flatcold'][0] if {'Attack', 'Claw'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Dagger Attacks": [dps['flatcold'][0] if {'Attack', 'Dagger'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Mace or Sceptre Attacks": [dps['flatcold'][0] if {'Attack', 'Mace'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Spells": [dps['flatcold'][0] if {'Spell'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Spells while Dual Wielding": [dps['flatcold'][0] if {'Spell', 'DualWielding'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Spells while holding a Shield": [dps['flatcold'][0] if {'Spell', 'Shield'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Spells while wielding a Two Handed Weapon": [dps['flatcold'][0] if {'Spell', 'TwoHandedWeapon'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Staff Attacks": [dps['flatcold'][0] if {'Attack', 'Staff'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Sword Attacks": [dps['flatcold'][0] if {'Attack', 'Sword'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Cold Damage to Wand Attacks": [dps['flatcold'][0] if {'Attack', 'Wand'}.issubset(selections) else 0, dps['flatcold'][1]],
		...
		"Adds # to # Fire Damage to Axe Attacks": [dps['flatfire'][0] if {'Attack', 'Axe'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Bow Attacks": [dps['flatfire'][0] if {'Attack', 'Bow'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Claw Attacks": [dps['flatfire'][0] if {'Attack', 'Claw'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Dagger Attacks": [dps['flatfire'][0] if {'Attack', 'Dagger'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Mace or Sceptre Attacks": [dps['flatfire'][0] if {'Attack', 'Mace'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Spells": [dps['flatfire'][0] if {'Spell'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Spells while Dual Wielding": [dps['flatfire'][0] if {'Spell', 'DualWielding'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Spells while holding a Shield": [dps['flatfire'][0] if {'Spell', 'Shield'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Spells while wielding a Two Handed Weapon": [dps['flatfire'][0] if {'Spell', 'TwoHandedWeapon'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Staff Attacks": [dps['flatfire'][0] if {'Attack', 'Staff'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Sword Attacks": [dps['flatfire'][0] if {'Attack', 'Sword'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage to Wand Attacks": [dps['flatfire'][0] if {'Attack', 'Wand'}.issubset(selections) else 0, dps['flatfire'][1]],
		...
		"Adds # to # Lightning Damage to Axe Attacks": [dps['flatlightning'][0] if {'Attack', 'Axe'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Bow Attacks": [dps['flatlightning'][0] if {'Attack', 'Bow'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Claw Attacks": [dps['flatlightning'][0] if {'Attack', 'Claw'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Dagger Attacks": [dps['flatlightning'][0] if {'Attack', 'Dagger'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Mace or Sceptre Attacks": [dps['flatlightning'][0] if {'Attack', 'Mace'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Spells": [dps['flatlightning'][0] if {'Spell'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Spells while Dual Wielding": [dps['flatlightning'][0] if {'Spell', 'DualWielding'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Spells while holding a Shield": [dps['flatlightning'][0] if {'Spell', 'Shield'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Spells while wielding a Two Handed Weapon": [dps['flatlightning'][0] if {'Spell', 'TwoHandedWeapon'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Staff Attacks": [dps['flatlightning'][0] if {'Attack', 'Staff'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Sword Attacks": [dps['flatlightning'][0] if {'Attack', 'Sword'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Lightning Damage to Wand Attacks": [dps['flatlightning'][0] if {'Attack', 'Wand'}.issubset(selections) else 0, dps['flatlightning'][1]],
		...
		"Adds # to # Physical Damage to Claw Attacks": [dps['flatphys'][0] if {'Attack', 'Claw'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Dagger Attacks": [dps['flatphys'][0] if {'Attack', 'Dagger'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Mace or Sceptre Attacks": [dps['flatphys'][0] if {'Attack', 'Mace'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Spells": [dps['flatphys'][0] if {'Spell'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Spells while Dual Wielding": [dps['flatphys'][0] if {'Spell', 'DualWielding'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Spells while holding a Shield": [dps['flatphys'][0] if {'Spell', 'Shield'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Spells while wielding a Two Handed Weapon": [dps['flatphys'][0] if {'Spell', 'TwoHandedWeapon'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Staff Attacks": [dps['flatphys'][0] if {'Attack', 'Staff'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Sword Attacks": [dps['flatphys'][0] if {'Attack', 'Sword'}.issubset(selections) else 0, dps['flatphys'][1]],
		"Adds # to # Physical Damage to Wand Attacks": [dps['flatphys'][0] if {'Attack', 'Wand'}.issubset(selections) else 0, dps['flatphys'][1]],
		# Flat Damage - Conditional
		"# to # added Fire Damage against Burning Enemies": [dps['flatfire'][0] if {'conditionEnemyBurning'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Fire Damage against Ignited Enemies": [dps['flatfire'][0] if {'conditionEnemyIgnited'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Cold Damage against Chilled or Frozen Enemies": [dps['flatcold'][0] if {'conditionEnemyChilled', 'conditionEnemyFrozen'}.intersection(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Lightning Damage against Shocked Enemies": [dps['flatlightning'][0] if {'conditionEnemyShocked'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Cold Damage if you've dealt a Critical Strike Recently": [dps['flatcold'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['flatcold'][1]],
		"Adds # to # Fire Damage if you've dealt a Critical Strike Recently": [dps['flatfire'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['flatfire'][1]],
		"Adds # to # Lightning Damage if you've dealt a Critical Strike Recently": [dps['flatlightning'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['flatlightning'][1]],
		"Adds # to # Physical Damage if you've dealt a Critical Strike Recently": [dps['flatphys'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['flatphys'][1]],
		# Damage Penetration
		...
		"Gain #% of Physical Damage as Extra Fire Damage if you've dealt a Critical Strike Recently": [dps['extrafire'][0] if {'conditionCritRecently'}.issubset(selections) else 0, dps['extrafire'][1]],
		...
		"Minions have #% increased Attack and Cast Speed if you or your Minions have Killed Recently": [dps['minionattackspeed'][0] + dps['minioncastspeed'][0] if {'conditionKilledRecently', 'conditionMinionsKilledRecently'}.intersection(selections) else 0, dps['minionattackspeed'][1] + dps['minioncastspeed'][1]],
		"Minions deal #% increased Damage if you've used a Minion Skill Recently": [dps['pminion'][0] if {'conditionUsedMinionSkillRecently'}.issubset(selections) else 0, dps['pminion'][1]],
		# Minion Flat Damage
		"Minions deal # to # additional Physical Damage": [dps['minionflatphys'][0], dps['minionflatphys'][1]],
		"Minions deal # to # Added Lightning Damage": [dps['minionflatlightning'][0], dps['minionflatlightning'][1]],
		...
		# Flasks
		"#% increased Critical Strike Chance during any Flask Effect": [dps['critchance'][0] if {'conditionUsingFlask'}.issubset(selections) else 0, dps['critchance'][1]],
		"#% to Critical Strike Multiplier during any Flask Effect": [dps['critmulti'][0] if {'conditionUsingFlask'}.issubset(selections) else 0, dps['critmulti'][1]],
		"Damage Penetrates #% Elemental Resistances during any Flask Effect": [dps['penall'][0] if {'conditionUsingFlask'}.issubset(selections) else 0, dps['penall'][1]],
		"#% increased Damage during any Flask Effect": [dps['pgeneric'][0] if {'conditionUsingFlask'}.issubset(selections) else 0, dps['pgeneric'][1]],
		"#% increased Attack Speed during any Flask Effect": [dps['attackspeed'][0] if {'conditionUsingFlask'}.issubset(selections) else 0, dps['attackspeed'][1]],
		"#% increased Cast Speed during any Flask Effect": [dps['castspeed'][0] if {'conditionUsingFlask'}.issubset(selections) else 0, dps['castspeed'][1]],
		"#% increased Melee Damage during any Flask Effect": [dps['pgeneric'][0] if {'conditionUsingFlask', 'Attack', 'Melee'}.issubset(selections) else 0, dps['pgeneric'][1]],
		"#% increased Spell Damage during any Flask Effect": [dps['pgeneric'][0] if {'conditionUsingFlask', 'Spell'}.issubset(selections) else 0, dps['pgeneric'][1]],
		"#% increased Critical Strike Chance against Blinded Enemies": [dps['critchance'][0] if {'conditionEnemyBlinded'}.issubset(selections) else 0, dps['critchance'][1]],
